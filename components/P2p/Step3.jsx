...
export const Step3 = ({ callback }) => {
  ...
  useEffect(() => {
    switch (FeeConfiguration?.case) {
      ...
      case 1:
        setSellerFee({
          token: FeeConfiguration?.fee_tokenContract, amount: ethers.utils.parseUnits((FeeConfiguration?.fee_amount ? FeeConfiguration?.fee_rate ?
            FeeConfiguration?.fee_amount > (userContext?.selectedActionsState?.p2p_fts?.reduce((total, i) => {
              return total + Number(i?.amount)
            }, 0) / 100) * FeeConfiguration?.fee_rate ? FeeConfiguration?.fee_amount :
              (userContext?.selectedActionsState?.p2p_fts?.reduce((total, i) => {
                return total + Number(i?.amount)
            ...
            FeeConfiguration?.fee_rate ?
              ...
              0).toString(), 18), to: FeeConfiguration?.fee_to
        });
        setBuyerFee({
          token: FeeConfiguration?.fee_tokenContract, amount: ethers.utils.parseUnits((FeeConfiguration?.fee_amount ? FeeConfiguration?.fee_rate ?
            FeeConfiguration?.fee_amount > (userContext?.selectedActionsState?.p2p_my_fts?.reduce((total, i) => {
              return total + Number(i?.amount)
            }, 0) / 100) * FeeConfiguration?.fee_rate ? FeeConfiguration?.fee_amount :
              (userContext?.selectedActionsState?.p2p_my_fts?.reduce((total, i) => {
                return total + Number(i?.amount)
              }, 0) / 100) * FeeConfiguration?.fee_rate :
            FeeConfiguration?.fee_amount :
            FeeConfiguration?.fee_rate ?
      ...
      case 3:
        if (userContext?.selectedActionsState?.p2p_my_fts?.length === 0 &&
          ...
          (userContext?.selectedActionsState?.p2p_fts?.[0]?.ft?.contract === ETHContract ||
            userContext?.selectedActionsState?.p2p_fts?.[0]?.contract === ETHContract)) {
          //Royalty
          setSellerFee({ token: constants.AddressZero, amount: 0, to: constants.AddressZero });
          setBuyerFee({ token: constants.AddressZero, amount: 0, to: constants.AddressZero });
          setRoyalty({ to: FeeConfiguration?.royalty_to, percentage: FeeConfiguration?.royalty_percentage });
        ...
        <Flex>
          <Box m="auto">
            ...
            {FeeConfiguration?.case !== 0 ?
              <Text textAlign={"center"} fontSize="12px" fontWeight={500} mt="15px">
                {
                  FeeConfiguration?.case === 1 ?
                    ...
                    FeeConfiguration?.case === 2 ? "Royalty:" + FeeConfiguration?.royalty_percentage?.map((i) => {
                      return userContext?.selectedActionsState?.p2p_fts?.reduce((total, i) => {
                        return total + Number(i?.amount)
                      }, 0) * i / 100 + "wETH";
                    }) : FeeConfiguration?.case === 3 && userContext?.selectedActionsState?.p2p_my_fts?.length === 0 &&
                      userContext?.selectedActionsState?.p2p_my_nfts?.length &&
                      userContext?.selectedActionsState?.p2p_my_nfts?.reduce((total, i) => {
                        if (i?.nft?.contract?.toLocaleLowerCase() === userContext?.selectedActionsState?.p2p_my_nfts?.[0]?.nft?.contract?.toLocaleLowerCase()) {
                          return total & true;
                      ...
                      userContext?.selectedActionsState?.p2p_fts?.length === 1 &&
                      (userContext?.selectedActionsState?.p2p_fts?.[0]?.ft?.contract === ETHContract ||
                        userContext?.selectedActionsState?.p2p_fts?.[0]?.contract === ETHContract) ? "Royalty:" + FeeConfiguration?.royalty_percentage?.map((i) => {
                          return userContext?.selectedActionsState?.p2p_fts?.reduce((total, i) => {
                            return total + Number(i?.amount)
                          }, 0) * i / 100 + "wETH";
                        }) : FeeConfiguration?.fee_amount > (userContext?.selectedActionsState?.p2p_my_fts?.reduce((total, i) => {
